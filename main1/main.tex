...
\author{Yagati Prem Sagar\\EE23BTECH11065}
\date{}
...
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.16 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.21 \pi
\end{align}
...
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.153\pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.228 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.2566$, $\Omega_{p2} = 0.342$ and $\Omega_{s1} = 0.2450$, $\Omega_{s2} = 0.3742$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.308$ and $B = \Omega_{p2} - \Omega_{p1} = 0.0854$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.591\\
...
we get $N\geq 4$ and $0.347\leq \epsilon \leq 0.61$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/Premsagar320/EE1205/tree/main/Audio_%20Filter/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{lstlisting}
https://github.com/Premsagar320/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Premsagar320/EE1205/blob/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.2474}{\brak{s_L-s_5}\brak{s_L-s_6}\brak{s_L-s_7}\brak{s_L-s_8}}\\
